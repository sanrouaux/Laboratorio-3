...
class UsuarioAPI implements IAPIUsable{    
    ...
    */
    public static function VerificaJWT($request, $response) {
        ...
        $retorno = new stdClass();
        $respuesta = Usuario::VerificaJWT($token);

        if($respuesta->exito) {
            $retorno->exito = true;
            $retorno->mensaje = 'Token valido';
            $retorno->perfil = $respuesta->perfil;
            $response = $response->withJson($retorno, 200);
        }
        else {
            $retorno->exito = false;
    ...
    * @param string $request con parametro 'id'=id del auto a eliminar
    * @param string $response 
    * @param array $args  
